...
2. **H2: Erros de Estado no Frontend**: O gerenciamento de visualizações e renderização no JavaScript da vitrine pode ter condições de corrida ou estados inconsistentes ao navegar rapidamente.
...
4. **H4: Problemas de Persistência no LocalStorage**: Dados corrompidos ou chaves inconsistentes entre páginas HTML e scripts da loja podem causar bugs de carrinho ou tema.
...
- [ ] Instrumentar `frontend/js/interacoes.js` para monitorar trocas de estado e erros de renderização.
