    ...
    function produtoCorrespondeBusca(produto, termoBusca) {
      ...
      return [
        ...
        produto.desc,
        produto.status
      ].some(campo => normalizarTextoBusca(campo).includes(termoBusca));
    }
    ...
    function renderizarProdutos() {
      ...
      const termoBusca = normalizarTextoBusca(document.getElementById('searchInput')?.value);
      ...
      if (termoBusca) filtered = filtered.filter(p => produtoCorrespondeBusca(p, termoBusca));
    ...
    window.buscarProdutos = renderizarProdutos;
